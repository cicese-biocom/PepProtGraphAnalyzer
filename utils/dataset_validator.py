...
class DatasetValidator(ABC):
    ...
    def transform(dataset) -> pd.DataFrame:
        ...
        sequence_df = sequence_df.assign(length=sequence_df['sequence'].str.len())
        sequence_df = sequence_df.sort_values(by='length', ascending=False).reset_index(drop=True)
        return sequence_df
...
class DatasetValidatorContext:
    ...
    def dataset_validator(self):
        return self._dataset_validator
