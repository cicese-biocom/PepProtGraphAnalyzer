...
def load_tertiary_structures(args: object) -> object:
    pdb_path, amino_acid_representation, output_setting, data = args
    ...
    with tqdm(range(len(data)), total=len(data), desc="Loading pdb files") as progress:
        pdbs = []
        ...
        if not sequences_to_exclude.empty:
            csv_file = output_setting['non_pdb_bound_sequences_file']
...
def predict_tertiary_structures(args):
    pdb_path, amino_acid_representation, data = args
    ...
    with tqdm(range(len(pdbs)), total=len(pdbs), desc="Saving pdb files", disable=False) as progress:
        for (pdb_name, pdb_str) in zip(pdb_names, pdbs):
            ...
            coordinates_matrix = \
                np.array(pdb_parser.get_atom_coordinates_from_pdb(pdb_str, amino_acid_representation),
                         dtype='float64')
